...
class TestDashboard:
    ...
    def test_dashboard_filter_by_time_period(self, populated_db):
        ...
        page_text = soup.get_text().lower()
        assert 'monthly' in page_text or 'bulanan' in page_text, "Harus menampilkan data dengan periode monthly"
        ...
        page_text = soup.get_text().lower()
        assert 'quarterly' in page_text or 'triwulanan' in page_text, "Harus menampilkan data dengan periode quarterly"
    ...
    def test_dashboard_no_results_filter(self, test_client):
        ...
        has_rows = len([row for row in table_rows if row.get_text(strip=True)]) > 1
        assert has_no_results_message or not has_rows, "Harus menampilkan pesan tidak ada hasil untuk filter"
