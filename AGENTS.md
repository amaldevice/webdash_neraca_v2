...
- When period mode is Triwulanan or the universal upload path is used, period fields should accept `YYYY`, `YYYY-MM`, `YYYY-Q1`, and `YYYY-MM-DD`-style inputs without unnecessary parse failures.
- For Upload/Unggah Data, user wants one universal long-form template: `Pengunggah`, dataset name/type, one `Indikator` column containing multi-dimensional parts separated by `|`, plus `Periode` and `Nilai`; `One Way`, `Two Way`, and `Three Way` are metadata, not separate Excel layouts.
...
- Upload / template code still keys on `dataset_slug`; treat `dataset_code` as migration terminology when touching upload/template flows.
- `scripts/services/` is an untracked mirror of `services/` and is unused by imports; avoid editing the copy in `scripts/` unless intentionally cleaning it up.
