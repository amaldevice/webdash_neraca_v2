...
## 2026-04-16 - Flush Isi Tabel Multi-DB

- Menambahkan utilitas `scripts/flush_db.py` untuk menghapus isi `data_entries` pada target `sqlite`, `mysql`, atau `both`.
- Script ini melakukan konfirmasi sebelum delete, mendukung mode `--dry-run`, dan mengecek jumlah baris sebelum/sesudah.
- `data_entries` dihapus melalui `DELETE`; skema DB dan tabel tetap dipertahankan.
- Update terbaru: run `python flush_db.py` kini default mode interaktif, menampilkan 3 opsi target (MySQL / SQLite / keduanya) setelah precheck koneksi/URL + cek file DB (SQLite) + validasi tabel `data_entries`.
- Mode hosting/server: file tetap dijalankan non-interaktif lewat `--targets` + `--yes`, precheck tetap memakai sumber URL dari env (`MYSQL_TARGET_URL`, `MIGRATE_TARGET_URL`, `DATABASE_URL`, `SQLITE_SOURCE_PATH`) dan fallback default `data.db`.
- Ditambahkan preload `.env` saat startup (`.env`, `DOTENV_PATH`) dan fallback baca `config.database_url()` supaya URL MySQL otomatis kebaca ketika script jalan dari cron/service.
- **Tambahan 2026-04-16 (opsional):** tambah script baru `scripts/migrate_mysql_to_sqlite.py` untuk backup `data_entries` dari source SQLAlchemy (MySQL/PostgreSQL) ke file SQLite; default output `backups/data_backup_<timestamp>.db`, supports `--truncate-target`/`--append`, `--dry-run`, dan verifikasi rows/sum.
...
- **Perubahan kode utama**: menghapus pemanggilan `fetch_aggregated_summary`/`refresh_aggregated_summary`, membersihkan alur upload/management agar tidak melakukan refresh cache agregat, menghapus model SQLAlchemy `aggregated_summary` dan utilitas `models/summary_store`.
- **Perubahan migrasi**: `alembic/versions/001_initial_schema.py` kini hanya membuat `data_entries`; `scripts/migrate_sqlite_to_mysql.py` tidak lagi memigrasi `aggregated_summary`.
- **Pembersihan tampilan**: menghapus template `_landing_summary_cards`, `_aggregated_plot_form`, `_aggregated_period_form`, `_script_aggregated_analysis`, dan `aggregated.html`; route/menu tidak lagi menaut ke halaman agregat.
- **Tes dan doc**: menyesuaikan test suite (`edge`, `concurrency`, `upload_flow`, `browse`, `alembic`, `security`, conftest) agar tidak lagi memvalidasi alur agregasi; memperbarui dokumen status rencana agar sinkron.
- **Perubahan terakhir**: Input `Versi/Waktu Unggah` pada form Unggah Data dan Input Manual dihilangkan; backend mengisi otomatis timestamp WITA. Tabel Pratinjau Data, Manajemen Data, dan ringkasan landing menampilkan waktu unggah dalam zona WITA.
...
- **Repo / CI (2026-04-15):** direktori **`.github/`** dihapus dari lokal dan remote (tidak ada lagi workflow GitHub Actions di tree); verifikasi lewat `pip install -r requirements-dev.txt` + `python -m pytest tests --ignore=tests/integration` (dan integrasi manual bila perlu).
  - **Persistence / SQLAlchemy multi-DB + CRUD (2026-04):** Baseline sama seperti eksplorasi awal (`sqlite3`, SQL tersebar, `ON CONFLICT`, integrity string-match). Rencana **`docs/superpowers/plans/2026-04-15-sqlalchemy-mysql-refactor.md`** + isu GitHub **https://github.com/amaldevice/webdash_neraca_v2/issues/9**. **P1 selesai:** dependensi SQLAlchemy/Alembic/PyMySQL di `requirements.txt`, `psycopg[binary]` di `requirements-dev.txt`, helper `database_url` / `use_sqlalchemy` di `config.py`, tes `tests/test_database_config.py`. **P2 selesai:** `infrastructure/db.py` (engine + `scoped_session` + `dispose_engine`), `create_app` memanggil `init_engine` / `register_flask_teardown` bila `DATABASE_URL` set, `dispose_engine` bila tidak; tes `test_create_app_sqlalchemy_engine_when_database_url_set`. **P3 selesai:** `infrastructure/orm_models.py` (mirror DDL), `alembic.ini` + `alembic/env.py` (DSN: `ALEMBIC_DATABASE_URL` → `DATABASE_URL` → `sqlite:///…/data.db`), revisi `001_initial_schema`, tes `tests/test_alembic_initial.py`, README bagian Alembic. **P4 (read-path) selesai:** `get_total_entries_count` / `query_data_entries` memakai SQLAlchemy bila `DATABASE_URL` + `init_engine`; `_row_to_public_dict` + `build_data_entry_filter_sqlalchemy`; `is_engine_initialized()` di `infrastructure/db.py`; tes `tests/test_queries_sqlalchemy.py`; `tests/test_mutations_baseline.py::test_upsert_entries_overwrites_existing_row` diselaraskan ke kunci unik non-NULL (SQLite UNIQUE + NULL). **P5 (write-path) selesai:** `infrastructure/dialect_upsert.py`; semua mutasi utama di `models/mutations.py` bercabang SA + `scoped_transaction()` di `infrastructure/db.py`; `app.create_app(..., init_sqlalchemy=...)` — default **tanpa** `init_engine` saat pytest (modul `app`); `services/upload_flow.py` menangkap `sqlalchemy.exc.IntegrityError`; tes `tests/test_mutations_sqlalchemy.py`; `tests/test_upload_flow.py` — `quarter` selaras `periods.parse_period_date` + asersi flash longgar. **P6 (browse):** cabang SA di `models/browse.py` difokuskan pada filter opsi; tes `tests/test_browse_summary_sqlalchemy.py`. **P7 selesai:** `services/db_errors.py` (`is_duplicate_key_error`, `resolve_duplicate_check_dialect`), `infrastructure/db.py` (`engine_dialect_name`), `services/upload_flow.py` memakai helper (bukan string-match ad hoc); `tests/test_db_errors.py` + regresi `tests/test_upload_flow.py` / `tests/test_bugs.py`. **Task 8 (langkah 1–2):** SQL dipindah ke `models/queries.py` — `fetch_series_for_comparison`, `preview_duplicates_batches`; `period_comparisons` / `upload_preview` tanpa `get_conn`. **Task 9 (langkah 1–3):** re-export `calculate_period_*` dihapus dari `models/__init__.py`; `routes/pages.py`, `services/period_analysis_export.py`, `tests/test_bugs.py` mengimpor `calculate_period_comparisons` dari `services.period_comparisons`. **Task 10 (langkah 1–4):** `.github/workflows/ci.yml` — matrix SQLite `pytest tests --ignore=tests/integration`; job `integration-mysql` / `integration-postgres` + `tests/integration/test_remote_dialect_smoke.py`; fixture `database_url` di `tests/conftest.py`; README contoh DSN. **Task 11 (langkah 1–2):** `scripts/migrate_sqlite_to_mysql.py` (ETL chunk + verifikasi count/sum), README subseksi migrasi. **Lanjutan pasca-commit:** `preview_duplicates_batches` cabang SA; `fetch_series_for_comparison` cabang SA + helper `period_analysis_range_sqlalchemy` (filter rentang agregat selaras `apply_period_range_filter` tanpa `time_period_filter`); signature query disederhanakan; tes `test_fetch_series_for_comparison_sa_then_legacy_same_file`; **PR #10 (2026-04-15):** konflik merge `main` vs `chore/remove-github-folder` hanya di `requirements-dev.txt` (main menambah `beautifulsoup4`, branch menambah `psycopg[binary]`); resolusi: **keduanya** dipertahankan. PR digabung ke `main`, branch remote `chore/remove-github-folder` dihapus. **Dotenv (2026-04-15):** dependensi `python-dotenv`; `config._load_dotenv_into_os_environ()` memuat `.env` + opsional `DOTENV_PATH` dengan `override=False`; `.env.example` + `.gitignore` untuk `.env`/`.env.local`. **Task 12 (selesai 2026-04-15):** persistence hanya SQLAlchemy (`get_conn` / cabang `sqlite3` di `models/` dihapus); `database_url()` + `init_engine` di app; `sqlite3` tersisa hanya untuk tipe pengecualian di `services/upload_flow.py` (selaras `IntegrityError`). Tes: `temp_db_path` di `tests/simple_tests/{functional,bug}_tests/conftest.py` di-scope **function** (satu file DB per tes) agar data `populated_db` tidak bocor ke `test_manual_entry`; `python -m pytest tests --ignore=tests/integration` hijau (satu `xfail` concurrency). Rincian: `docs/superpowers/plans/2026-04-15-sqlalchemy-mysql-refactor.md`. **Task 13:** `EntryListParams` di `services/list_view.py`, `models/repositories/entry_list.py`, route `pages`/`manage` memakai repository baca; tes `test_data_management_actions` diperbaiki (nilai dalam rentang filter). Rencana: `docs/superpowers/plans/2026-04-15-sqlalchemy-mysql-refactor.md`.
...
- **Refactor Python lanjutan (2026-03, wave 3):** **Telah dihapus** pada sesi cleanup repository: `services/aggregation.py` dan `aggregator.py` (shim kompatibilitas) setelah fitur `Ringkasan Agregat` disahkan untuk ditiadakan sementara.
- **Refactor Python lanjutan (2026-03, wave 2)**: `services/period_filters.py` memuat parsing rentang periode + `apply_period_range_filter` (dipakai `get_total_entries_count`, `query_data_entries`, `delete_data_by_filter`). `services/period_comparisons.py` memuat seluruh analitik Q/Q, M/M, Y/Y, YTD, C→C; data deret diambil lewat `models.queries.fetch_series_for_comparison`. **(2026-04)** `models/__init__.py` tidak lagi re-export `calculate_period_*` — route/export/impor dari `services.period_comparisons`.
