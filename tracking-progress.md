...
## Current Feature Set
- **Landing dashboard**: Aggregated cards show flow/stock summaries plus metadata (uploader, version), with quick actions that open upload, manual input, or aggregated summary views.
...
- **Dashboard table + filters**: Filter persisted entries by data type and time period, display the latest 50 rows, and keep the view refreshed via cached aggregated summaries.
- **Raw exports**: Export the filtered rows before aggregation as CSV or Excel straight from the dashboard filters.

## Feature Change Log

### Latest State (after current round of work)
- **Aggregation cache + summary refresh**: Every insert triggers `refresh_aggregated_summary()` that recomputes and stores JSON summaries; landing/dashboard read the cached snapshot for consistent cards.
...
- **Landing + aggregated views**: Initial dashboards rendered aggregated summaries and placeholder tables before filtering/export functionality existed.
