...
class TestManualEntry:
    ...
    def test_manual_entry_valid_monthly_data(self, test_client):
        ...
        assert response.status_code == 302, "Input berhasil harus redirect"
    ...
    def test_manual_entry_invalid_period_date_format(self, test_client):
        ...
        assert response.status_code == 302, "Format tanggal salah harus redirect dengan error"
        response = test_client.get('/manual', follow_redirects=True)
        soup = BeautifulSoup(response.data, 'html.parser')
        error_text = soup.get_text().lower()
        assert 'valid' in error_text or 'tanggal' in error_text or 'date' in error_text, \
            "Harus menampilkan error untuk format tanggal yang salah"
    ...
    def test_manual_entry_handles_edge_cases(self, test_client):
        ...
        assert response.status_code == 302, "Nilai desimal kecil harus diterima"
        ...
        assert response.status_code == 302, "Nilai negatif harus diterima jika valid untuk bisnis"
    ...
    def test_manual_entry_database_integrity(self, test_client):
        ...
        assert response.status_code == 302, "Input pertama harus berhasil"
        ...
        assert response.status_code == 302, "Input dengan bulan berbeda harus berhasil"
