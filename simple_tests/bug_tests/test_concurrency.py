...
class TestConcurrency:
    ...
    def test_file_system_concurrent_writes(self, test_client):
        ...
        for status, worker_id in results:
            assert status == 302, f"File upload {worker_id} failed with status {status}"

        # Check that all files were created
        import os
        uploads_dir = 'uploads'
        if os.path.exists(uploads_dir):
            uploaded_files = [f for f in os.listdir(uploads_dir) if f.startswith('concurrent_')]
            assert len(uploaded_files) == 5, f"Expected 5 uploaded files, got {len(uploaded_files)}"
