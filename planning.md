...
- **Logging & audit (2026-04)**: `configure_flask_app` memanggil `configure_logging` (`LOG_LEVEL`, `LOG_FORMAT` teks atau `json`). `create_app` men-set `g.request_id`, header `X-Request-ID`, dan access log ringkas per respons. Event bisnis memakai logger `audit` lewat `services/audit_log.log_audit` (data management, upload/manual, preview session, ekspor); mutasi DB men-log `db_mutation_ok` di `models/mutations`; agregasi men-log durasi/ukuran di `services/aggregation.py` dan byte JSON di `models/summary_store.save_aggregated_summary`. Rencana sumber: `docs/superpowers/plans/2026-04-13-logging-and-mysql-migration.md` (Bagian A).
